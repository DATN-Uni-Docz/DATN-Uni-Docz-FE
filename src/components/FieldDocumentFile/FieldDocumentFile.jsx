...
const RemoveFileButton = (props) => {
  const { className, rootClassName, onClick } = props;
  const classes = classNames(rootClassName || css.removeFile, className);
  return (
...
// Component that shows certifications from "files" field array
const FieldDocumentFile = (props) => {
  const { name, intl, onRemoveFile, editMode = false } = props;
          ...
          <div className={css.filePreview}>
            <DocumentFile
              ...
              className={classNames(css.thumbnail, {
                [css.disabled]: editMode,
              })}
              savedFileAltText={intl.formatMessage({
                id: "FieldDocumentFile.savedFileAltText",
              })}
            />
            {!editMode && (
              <RemoveFileButton onClick={() => onRemoveFile(file)} />
            )}
          </div>
        ) : null;
      }}
...
export default FieldDocumentFile;
